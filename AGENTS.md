...
- Fútbol results auto-update via `/api/cron/scores` (two-source agreement) into `match_results`; don't hand-edit scores. The coffee product is now a Juan Valdez resale (not our own single-origin), keep its copy honest.
